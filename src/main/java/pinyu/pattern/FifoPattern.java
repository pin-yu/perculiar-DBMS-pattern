 ...
 * will be notified and the behavior contradict the conservative locking protocol.
 * The workaround in Elasql is to use a queue to maintain the
 * request order, and the waken thread will check the queue to see whether it
 * has the right to get the resource. These wake-then-check behavior causes quite
 * a few overhead.
 * 
 ...
 * object of the queue.
 */
public class FifoPattern implements Runnable {
	public static ConcurrentLinkedQueue<Object> requestQueue = new ConcurrentLinkedQueue<Object>();
	public static AtomicInteger txNum = new AtomicInteger(1);

	public void run() {
		int localTxNum = txNum.getAndIncrement();
		...
		while (true) {
			...
			while (requestQueue.peek() != dummyObj) {
				synchronized (dummyObj) {
					try {
						System.out.println("Tx " + localTxNum + " waits");
						dummyObj.wait();
						System.out.println("Tx " + localTxNum + " is waked up");
			...
			System.out.println("Tx " + localTxNum + " acquires the lock");
			...
			requestQueue.remove();
			System.out.println("Tx " + localTxNum + " releases the lock");

			Object otherTxObj = requestQueue.peek();
			synchronized (otherTxObj) {
				System.out.println("Tx " + localTxNum + " notifies other tx");
				otherTxObj.notify();
			}
